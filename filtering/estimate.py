...
class EKF():
    ...
    def getR(self, learning_class, z):
        ...
        mu_tensor = utils.numpy2torch(mu_phys, learning_class.device)
        sigma_tensor = utils.numpy2torch(self.sigma,learning_class.device)
        H_tensor = utils.numpy2torch(self.H, learning_class.device)
        dronegate_bounds_torch = utils.numpy2torch(self.dronegate_bounds, learning_class.device)
        dronegate_minimum_torch = utils.numpy2torch(self.dronegate_minimum, learning_class.device)
        z_tensor = utils.numpy2torch(z, learning_class.device)
        ...
        if len(p_batch_out_norm1) != 2:
            ...
            R_tensor = (1.0/(2*N))*torch.sum(slist,dim=0)  
    ...
    # Simulate one forward pass of the EKF with GPS
    def unit_test(self):
        ...
        z = np.ones(self.h)
        R = np.identity(self.h)
        self.ekf(u, z, None, True)

def main():
    ekf_class = EKF(9, 4, 6, 0.01)
    ekf_class.unit_test()
    print("mu: ", ekf_class.mu)
    print("sigma: ", ekf_class.sigma)
