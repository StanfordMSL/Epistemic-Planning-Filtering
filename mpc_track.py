...
def track_traj(x0, k, x_traj, u_init):
    cost = 2000.
    iter = 0
    ...
    x0_in = (x_traj[0:6,:] * dronegate_bounds.reshape((-1,1))) + dronegate_minimum.reshape((-1,1))
    x0_in = np.vstack((x0_in, x_traj[6::, :]))
    x0_in[:,0] = x0

    x0_in = x0_in.T.flatten()
    u0_in = u_init.T.flatten()
    #print("initialization")
    pconst = 1.0
    CQ = casadi.SX(np.identity(9*(k-1)))
    ...
    # cost function
    cost = 0.5*(1./(k-1))*casadi.mtimes(casadi.transpose((x[9::]-x0_in[9::])),casadi.mtimes(CQ,(x[9::]-x0_in[9::]))) + 0.5*(1./(k-1))*casadi.mtimes(casadi.transpose(u), casadi.mtimes(CR,u))
...
# k = 200
# x0 = np.array([1.0, 6.0, 0.0,  0.0, 0.0, -1.57, 0.0, 0.0, 0.0])
# u0 = np.array([9.81, 0.0, 0.0, 0.0])
# x_traj = np.tile(x0.reshape((9,-1)), k)
# u_init = np.tile(u0.reshape((4,-1)), k-1)
# # track_traj(x0, k, x_traj, u_init)
# x = x0.copy()
